...
class TestQueryEncoderAttention:
    ...
    def test_sol_mode_no_causal_factor(self, comprehensive_perf_db):
        ...
        sol_math = (
            ops / comprehensive_perf_db.system_spec["gpu"]["bfloat16_tc_flops"]
            * 1000 / fmha.value.compute
        )
...
class TestEncoderAttentionOp:
    ...
    def test_scale_factor_multiplies_latency(self, comprehensive_perf_db):
        ...
        depth = 32
        op_single = EncoderAttention(
            "encoder_attention", 1.0, 16, 72, partial_rotary_factor=0.0
        )
        op_depth = EncoderAttention(
            "encoder_attention", depth, 16, 72, partial_rotary_factor=0.0
        )
