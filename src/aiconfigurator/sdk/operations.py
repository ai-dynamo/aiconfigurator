...
class MegaMoECommunicationEstimate:
    ...
    ``overlappable_ms`` is the hot-path communication term that belongs under
    ``max(core_compute_ms, overlappable_ms)``. ``tail_ms`` is reported
    separately for callers that model a cleanup/sync tail outside that max.
    """
...
def build_route_matrix(
    ...
    *,
    ...
    """Build routes[src_rank][dst_rank] from per-source-rank top-k IDs."""
...
def estimate_megamoe_traffic(
    ...
    *,
    ...
    Primary bytes follow DeepGEMM's benchmark convention: remote dispatch pulls
    FP8 activation (``hidden`` bytes) and combine writes BF16 output
    (``2 * hidden`` bytes), so one remote token/expert selection costs
    ``3 * hidden`` primary bytes.
    """
    if hidden_size <= 0:
...
def compose_megamoe_routed_latency_ms(
    *,
    ...
    comm_estimate: Optional[MegaMoECommunicationEstimate] = None,
) -> float:
...
class Dsv4MegaMoEDispatch(Operation):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._nvl_barrier_latency_us = nvl_barrier_latency_us
        self._nvl_barrier_count = nvl_barrier_count
        self._cleanup_barrier_count = cleanup_barrier_count
    ...
    def query(self, database: PerfDatabase, **kwargs) -> PerformanceResult:
        route_matrix = kwargs.get("route_matrix")
        if route_matrix is None:
            raise ValueError("Dsv4MegaMoEDispatch requires route_matrix in query kwargs")

        num_tokens_per_rank = kwargs.get("num_tokens_per_rank")
        if num_tokens_per_rank is None:
            num_tokens_per_rank = kwargs.get("x")
        if num_tokens_per_rank is None:
            raise ValueError("Dsv4MegaMoEDispatch requires num_tokens_per_rank or x in query kwargs")
        ...
        comm = estimate_megamoe_communication_from_perf_database_ms(
            ...
            nvl_barrier_latency_us=self._nvl_barrier_latency_us,
            nvl_barrier_count=self._nvl_barrier_count,
            cleanup_barrier_count=self._cleanup_barrier_count,
            include_metadata=self._include_metadata,
            quant_group_size=self._quant_group_size,
        )
        latency = comm.overlappable_ms + (comm.tail_ms if self._include_tail else 0.0)
