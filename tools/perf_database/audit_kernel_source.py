...
def render_markdown(summaries: list[dict]) -> str:
    ...
    lines.append(
        ...
        "directories (cross-version and cross-backend) when the database is loaded in SILICON or HYBRID mode.\n"
        "- **`shared_fallback`** — `kernel_source = default`. Framework-implicit, low-fidelity. "
        "Inherited only in HYBRID mode (HYBRID already accepts coarser fallbacks).\n"
        "\n"
...
def render_manifest(summaries: list[dict]) -> str:
    ...
    lines: list[str] = [
        ...
        "#   construction time. When the database is loaded in SILICON or HYBRID mode,",
        "#   the loader walks sibling <sys>/<framework>/<version>/<op_file> directories",
        "#   and inherits rows tagged with the kernel_sources this manifest declares the",
        "#   active backend may consume — keeping the active backend's own rows on key",
        "#   conflict. SILICON admits only exact `tier=shared` named-kernel rows; HYBRID",
        "#   also admits `tier=shared_fallback` (kernel_source=default,",
        "#   framework-implicit) rows because HYBRID already accepts coarser fallbacks.",
        "#",
        "# How to regenerate:",
