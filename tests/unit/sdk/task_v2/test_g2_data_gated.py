...
moment its ``moe_a2a_perf`` parquet exists for a system. Local expert compute
is modeled from stock ``moe_perf`` with no family-specific compute table. On
sglang AND vllm:
...
from aiconfigurator.sdk.operations import ModeledEPMoE
from aiconfigurator.sdk.operations.moe_comm import MoEAllToAll
...
def synth_systems(tmp_path):
    """A synthetic systems root (8 GPUs/node, SM 90) holding ONLY the
    large-EP A2A table, for sglang AND vllm, mounted alongside the shipped
...
def _large_ep_ops(op_list) -> list:
    """All measured-A2A/modeled-compute ops, recursing into OverlapOp groups."""
    found = []
    stack = list(op_list)
    while stack:
        op = stack.pop()
        if isinstance(op, (MoEAllToAll, ModeledEPMoE)):
            found.append(op)
