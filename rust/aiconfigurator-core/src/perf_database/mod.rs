...
impl PerfDatabase {
    ...
    pub fn load(
        ...
        let data_root = systems_root.join(&spec.data_dir).join(backend).join(version);
        if !data_root.is_dir() {
        ...
        Ok(Self {
            ...
            system_spec: spec,
            gemm: GemmTable::new(data_root.clone()),
            attention: AttentionTable::new(data_root.clone()),
            mla: MlaTable::new(data_root.clone()),
            moe: MoeTable::new(data_root.clone()),
            communication: CommunicationTable::new(data_root.clone()),
