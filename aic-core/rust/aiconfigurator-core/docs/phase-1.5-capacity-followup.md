...
- **Rust top-level fn** `aiconfigurator_core::estimate_kv_cache(req)` —
  `aic-core/rust/aiconfigurator-core/src/memory.rs`. A **pure** PyO3 forwarder for
...
- **Rust forwarder round-trip** (`fetch_python_estimate` forwarding
  ...
  `aic-core/rust/aiconfigurator-core/tests/memory_round_trip.rs`, an enforced end-to-end
