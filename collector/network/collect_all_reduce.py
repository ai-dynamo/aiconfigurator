...
DEFAULT_RMS_HIDDEN_SIZES = (2048, 4096, 5120, 7168, 8192)


def parse_rms_hidden_sizes(hidden_sizes: str | None) -> list[int]:
    if not hidden_sizes:
        return list(DEFAULT_RMS_HIDDEN_SIZES)
    parsed = [int(value.strip()) for value in hidden_sizes.split(",") if value.strip()]
    if not parsed:
        raise ValueError("At least one hidden size is required for allreduce_rms collection")
...
def _benchmark_vllm_allreduce_rms_shape(
    *,
    ...
    del dtype
    input_tensors = [
        torch.ones((num_tokens, hidden_size), dtype=torch_dtype, device=get_device_str()) for _ in range(repeat_n)
    ]
    residual_tensors = [
        torch.ones((num_tokens, hidden_size), dtype=torch_dtype, device=get_device_str()) for _ in range(repeat_n)
    ]
...
def benchmark_vllm_allreduce_rms(
    ...
    Collects one message-size sweep per hidden size. The SDK query path keeps
    ``num_tokens`` fixed and interpolates across the collected hidden-size
    buckets, so models like Qwen3.6 (hidden_size=2048) are not forced onto a
    4096-only surface.
    """
    setup_vllm_distributed(world_size, rank, use_slurm)

    for hidden_size in hidden_sizes or list(DEFAULT_RMS_HIDDEN_SIZES):
        _benchmark_vllm_allreduce_rms_hidden_size(
            dtype=dtype,
...
def _benchmark_vllm_allreduce_rms_hidden_size(
    *,
    dtype: str,
    test_range: str,
    world_size: int,
        ...
        for use_graph in flag_lst:
            ...
            if rank == 0:
                log_perf(
                    item_list=[
                        {
                            ...
                            "shape_policy": f"hidden_size_{hidden_size}",
                            "max_token_num": max_token_num,
                            "backend": f"vllm_{mode_str}",
                        }
                    ],
                    ...
                    kernel_source=f"vLLM_allreduce_rms_{mode_str}",
                    perf_filename=perf_filename,
                    power_stats=power_stats,
                )
    ...
    if rank == 0:
        print(f"[vLLM-allreduce-rms] Finished hidden_size={hidden_size} sweep")
...
def allreduce_benchmark(
    ...
    power_min_duration: float = 1.0,
    op: str = "allreduce",
    rms_hidden_sizes: list[int] | None = None,
):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rms-hidden-sizes",
        type=str,
        default=",".join(str(value) for value in DEFAULT_RMS_HIDDEN_SIZES),
        help=(
            "Comma-separated hidden sizes for vLLM allreduce_rms collection. "
            f"Default: {','.join(str(value) for value in DEFAULT_RMS_HIDDEN_SIZES)}."
        ),
    )
    ...
    allreduce_benchmark(
        ...
        args.op,
        parse_rms_hidden_sizes(args.rms_hidden_sizes),
    )
