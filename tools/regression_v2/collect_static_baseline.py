...
"""Collect (or refresh) the tier-1 run_static baseline.
...
layer OFF, and writes one CSV per combo.

Every grid row always produces exactly one output row with a three-state
status, so baselines are rectangular and diffs are reviewable:
...
Usage:
  # PR profile (latest data-carrying version per system/backend), write baselines:
  python tools/regression_v2/collect_static_baseline.py --update

  # Recompute into a scratch dir for comparison (CI does this):
  python tools/regression_v2/collect_static_baseline.py --output-dir /tmp/regv2-current

  # Restrict scope while iterating:
  python tools/regression_v2/collect_static_baseline.py --systems h200_sxm --backends trtllm --jobs 1
"""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Where to write combo CSVs. Defaults to a scratch dir unless --update.",
    )
    parser.add_argument(
        "--update",
        action="store_true",
        help=f"Write into the committed baseline dir ({grid.DEFAULT_BASELINE_DIR}).",
    )
    args = parser.parse_args()

    if args.update and args.output_dir:
        parser.error("--update and --output-dir are mutually exclusive")
    output_dir = grid.DEFAULT_BASELINE_DIR if args.update else (args.output_dir or Path("regv2_current"))
