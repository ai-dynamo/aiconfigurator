...
def _cfg(**search_space):
    ...
    return SmartSearchConfig(
        search_space=ss,
        workload={"isl": 4000, "osl": 1000, "request_rate": 25, "num_request_ratio": 40},  # static
    )
...
def test_evaluate_preset_constant_last_over_window_list(monkeypatch):
    ...
    monkeypatch.setattr(lps, "_load_predictors", lambda: {"constant": _LastValuePredictor})
...
def test_evaluate_preset_empty_windows_is_inf(monkeypatch):
    ...
    assert evaluate_preset([], {"family": "constant", "log1p": False}, 180, warmup=0) == math.inf
...
def test_sweep_falls_back_to_constant_last_when_no_winner(monkeypatch):
    ...
    monkeypatch.setattr(lps, "build_windows", lambda _path, _iv: [])  # empty window list
    monkeypatch.setattr(lps, "_common_warmup", lambda _presets, _iv: 0)
    monkeypatch.setattr(lps, "evaluate_preset", lambda *a, **k: math.inf)  # all presets tie at inf

    r = sweep_load_predictor(_trace_cfg(planner_scaling_policy=["throughput_180_5"]), show_progress=False)
