...
"""Phase 4b-1: prove the typed k8s builders emit the four hardware/transport
...
def _services(name):
    ...
    arts = generate_backend_artifacts(
        copy.deepcopy(c.params), c.backend, backend_version=c.backend_version
    )
    doc = [
        d
...
def test_h100_empty_tolerations_not_emitted():
    ...
    docs = build_dgd(ctx, "vllm", resolved_facts=_Facts())
