...
`tests/integration/cross_package/`. After Phase 3 (C8-C10) all 12
...
`tests/integration/cross_package/benchmark_engine_step.py`.
...
3. Capture a baseline benchmark snapshot to
   `tests/integration/cross_package/benchmarks.md` so Phase 3 progress
...
AICONFIGURATOR_RUST_CORE_AUTOBUILD=1 uv run pytest \
  tests/integration/cross_package/test_engine_step_parity.py -p no:xdist
```
...
Phase 3 profiling (recorded in `tests/integration/cross_package/benchmarks.md`) attributes
