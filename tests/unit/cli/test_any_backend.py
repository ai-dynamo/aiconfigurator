...
class TestAnyBackendEnum:
    ...
    def test_concrete_backends_excludes_any(self):
        """Test that CONCRETE_BACKENDS does not include 'any'."""
        assert common.BackendName.any not in common.CONCRETE_BACKENDS
        assert len(common.CONCRETE_BACKENDS) == 3
        assert common.BackendName.trtllm in common.CONCRETE_BACKENDS
        assert common.BackendName.sglang in common.CONCRETE_BACKENDS
        assert common.BackendName.vllm in common.CONCRETE_BACKENDS


class TestGetBackendsToCheck:
    """Test the _get_backends_to_check helper function."""

    def test_any_expands_to_all_backends(self):
        """Test that 'any' expands to all concrete backends."""
        backends = _get_backends_to_check("any")
    ...
    def test_single_backend_returns_single_list(self):
        ...
        assert _get_backends_to_check("trtllm") == ["trtllm"]
        assert _get_backends_to_check("sglang") == ["sglang"]
        assert _get_backends_to_check("vllm") == ["vllm"]
