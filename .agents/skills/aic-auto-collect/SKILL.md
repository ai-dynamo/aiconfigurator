...
mkdir -p "$COLLECTOR_LOG_DIR"
python packages/aiconfigurator/collector/collect.py --backend <backend> --ops <op> --smoke
python packages/aiconfigurator/collector/collect.py --backend <backend> --ops <op> \
...
2. Inspect `packages/aiconfigurator/collector/<backend>/registry.py` for version routes and the concrete `collect_xx.py` files registered for this version.
...
```bash
python3 packages/aiconfigurator/collector/collect.py --backend <backend> --ops <op> --smoke
python3 packages/aiconfigurator/collector/collect.py --backend <backend> --ops <op> --shuffle --limit 20
python3 packages/aiconfigurator/collector/collect.py --backend <backend> --ops <op> --shuffle --limit 100
python3 packages/aiconfigurator/collector/collect.py --backend <backend> --ops <op> \
  --checkpoint-dir "$COLLECTOR_CHECKPOINT_DIR/<backend>/<version>/<op>" --resume
```
...
- For API/import/signature errors:
  1. Read `packages/aiconfigurator/collector/<backend>/registry.py` to map the failing op to its collector module.
  2. Read the failing `packages/aiconfigurator/collector/<backend>/collect_*.py`.
...
- If a collector route only exists for future framework versions, use `VersionRoute` in `packages/aiconfigurator/collector/<backend>/registry.py` and add a wrapper module with an explicit `__compat__`.
...
- If `sglang` is partially installed in a unit-test image, `sglang.srt...` probes can raise. Runtime skip checks belong in `packages/aiconfigurator/collector/sglang/collect_*.py` wrappers, not necessarily in `packages/aiconfigurator/collector/common_test_cases.py`.
