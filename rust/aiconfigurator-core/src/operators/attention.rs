...
impl EncoderAttentionOp {
    ...
    pub fn query(
        ...
        let (latency, source) = query_encoder_attention_table(
            db,
            batch_size,
            s,
            self.n,
...
mod tests {
    ...
    fn mem_op_latency_uses_empirical_formula() {
        let db = b200_vllm_db();
        // b200_sxm.yaml: mem_bw=7.7e12 (per-datasheet fix, PR #1246),
        // scaling=0.8, constant=3e-6.
        // For 1MB: latency = (1e6 / (7.7e12 * 0.8) + 3e-6) * 1000 ≈ 0.165ms.
        let latency = mem_op_latency_ms(&db.system_spec, 1_000_000.0);
        let expected = (1_000_000.0_f64 / (7.7e12 * 0.8) + 3e-6) * 1000.0;
