...
"""Unit tests for pipeline-parallel (PP) schedule modeling.

Covers ``aiconfigurator_core.sdk.pipeline``:

- op -> stage placement classification (first / last / link / per-layer),
- layer partitioning, including the uneven ``num_layers % pp_size != 0`` case
  that previously only produced a "we're nothing to correct this" warning,
- ``balance_factor``: the pipeline advances at ``max_i(t_i)``, not the average,
- ``fill_factor``: pipe starvation when in-flight microbatches < stages,
- the ``pp_size == 1`` identity that keeps single-stage results unchanged.
"""
...
def test_explicit_partition_must_match_pp_size():
    with pytest.raises(ValueError, match="partition has 3 entries"):
        PipelineSchedule(pp_size=4, partition=(16, 16, 32))


def test_explicit_partition_is_honored():
    # Front-load layers to compensate for the head on the last stage.
    sched = PipelineSchedule(pp_size=2, partition=(36, 28))
    assert sched.layer_partition(_NUM_LAYERS) == (36, 28)
...
def test_single_stage_is_exactly_neutral():
    sched = PipelineSchedule(pp_size=1)
    assert sched.balance_factor(_STEP, _NUM_LAYERS) == 1.0
    assert sched.fill_factor() == 1.0
    assert sched.efficiency(_STEP, _NUM_LAYERS) == 1.0
    assert sched.per_hop_latency(_STEP) == 0.0


def test_empty_step_is_neutral():
    assert PipelineSchedule(pp_size=8).balance_factor({}, _NUM_LAYERS) == 1.0


# ---------------------------------------------------------------------------
# Stage times and the max-not-average cycle
# ---------------------------------------------------------------------------


def test_stage_times_place_head_and_embedding():
    stages = PipelineSchedule(pp_size=8).stage_times(_STEP, _NUM_LAYERS)
...
def test_cycle_is_set_by_fattest_stage_not_the_average():
    sched = PipelineSchedule(pp_size=8)
    step_total = sum(_STEP.values())  # 72.5 ms
    ideal_cycle = step_total / 8  # 9.0625 ms
    cycle = sched.cycle_time(_STEP, _NUM_LAYERS)  # 8.0 + 8.0 = 16.0 ms
    assert cycle == pytest.approx(16.0)
    assert cycle > ideal_cycle
    assert sched.balance_factor(_STEP, _NUM_LAYERS) == pytest.approx(ideal_cycle / cycle)


def test_balance_degrades_as_stages_multiply():
    factors = [PipelineSchedule(pp_size=pp).balance_factor(_STEP, _NUM_LAYERS) for pp in (1, 2, 4, 8)]
...
def test_uneven_layer_split_costs_throughput():
    ...
    sched = PipelineSchedule(pp_size=6)
    # Ideal would be 64/6 layers per stage; reality is 11.
    assert sched.balance_factor(balanced_only, _NUM_LAYERS) == pytest.approx((64 / 6) / 11)


def test_perfectly_balanced_step_keeps_full_speedup():
    """A step with no first/last-stage ops and an even split loses nothing."""
    sched = PipelineSchedule(pp_size=8)
    assert sched.balance_factor({"generation_qkv_gemm": _PER_LAYER_MS}, _NUM_LAYERS) == pytest.approx(1.0)
...
def test_link_latency_is_excluded_from_stage_times_and_amortized_per_hop():
    ...
    assert sched.stage_times(step, _NUM_LAYERS) == sched.stage_times(_STEP, _NUM_LAYERS)
    # ...but one hop is charged to the cycle.
    assert sched.per_hop_latency(step) == pytest.approx(1.0)
    assert sched.cycle_time(step, _NUM_LAYERS) == pytest.approx(16.0 + 1.0)


def test_p2p_overlap_hides_the_hop():
    step = dict(_STEP, generation_p2p=7.0)
    overlapped = PipelineSchedule(pp_size=8, p2p_overlap=True)
    assert overlapped.cycle_time(step, _NUM_LAYERS) == pytest.approx(16.0)
...
def test_fill_factor_saturates_at_pp_size(m, expected):
    assert PipelineSchedule(pp_size=8, num_microbatches=m).fill_factor() == pytest.approx(expected)


def test_default_microbatches_fills_the_pipe():
    """None reproduces the historical 'always exactly full' assumption."""
    assert PipelineSchedule(pp_size=8).fill_factor() == 1.0


def test_efficiency_is_balance_times_fill():
    sched = PipelineSchedule(pp_size=8, num_microbatches=4)
    assert sched.efficiency(_STEP, _NUM_LAYERS) == pytest.approx(
        sched.balance_factor(_STEP, _NUM_LAYERS) * 0.5
    )
