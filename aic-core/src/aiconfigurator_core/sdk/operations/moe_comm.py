...
class EPMoE(Operation):
    ...
    def query(self, database: PerfDatabase, **kwargs) -> PerformanceResult:
        ...
        if self._enable_eplb and self._inference_phase == "context" and kernel_source in _SGLANG_ADAPTED_KERNEL_SOURCES:
            # Legacy fidelity: the sglang MoE silicon query corrects tokens to
            # ``int(num_tokens * 0.8)`` when eplb is enabled during prefill,
            # before the table lookup. sglang-adapted legs only —
            # TrtLLMWideEPMoE never applied the 0.8 (its EPLB effect rides the
            # ``_eplb`` distribution suffix).
