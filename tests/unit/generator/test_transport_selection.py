...
def _params(transport=None):
    p = {"ServiceConfig": {"model_path": "Qwen/Qwen3-0.6B"}, "K8sConfig": {"system_name": "gb200"}, "NodeConfig": {}}
...
def test_unknown_transport_degrades_gracefully():
    ...
    p = build_naive_generator_params(
...
def test_ib_transport_env_reaches_worker_pod():
    ...
    arts = generate_backend_artifacts(copy.deepcopy(case.params), case.backend, backend_version=case.backend_version)
    doc = [d for d in yaml.safe_load_all(arts["k8s_deploy.yaml"]) if d and d.get("kind") == "DynamoGraphDeployment"][0]
    worker = next(v for k, v in doc["spec"]["services"].items() if "frontend" not in k.lower())
