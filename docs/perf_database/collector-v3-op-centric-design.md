...
- Path-aware consumers to sweep in the same PR: loader discovery,
  `tools/perf_database/generate_op_kernel_source_manifest.py`, `tools/perf_database/parquet_diff.py`,
...
One audit tool (sibling of `generate_op_kernel_source_manifest.py`), run on every PR touching
