...
def test_resolve_returns_hardware_model_and_runtime_facts():
    facts = resolve_facts(
        model_profile_id="deepseek-v4", hardware="gb200",
        transport="nvlink", dynamo_version="1.2.0", backend="trtllm",
    )
...
def test_unknown_model_resolves_to_none_not_error():
    facts = resolve_facts(
        model_profile_id=None, hardware="h200",
        transport="nvlink", dynamo_version="1.2.0", backend="vllm",
    )
...
def test_unknown_hardware_raises_clear_error():
    with pytest.raises(Exception):
        resolve_facts(model_profile_id=None, hardware="nonexistent",
                      transport="nvlink", dynamo_version="1.2.0", backend="vllm")
