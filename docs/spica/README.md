...
**linux-x86_64 box with an NVIDIA GPU**, add the matching CUDA plugin to run the
...
The JAX CUDA wheels exist **only for linux-x86_64**
(not macOS / Windows / aarch64 — those use the CPU-only `spica` extra). With no GPU present
