...
def _is_token_id_list(x) -> bool:
    """Return True if x looks like a list/tuple of token ids (ints)."""
    if isinstance(x, (list, tuple)):
        return len(x) == 0 or isinstance(x[0], int)
    return False

def _decode_ids_if_needed(prompt):
    """Decode token ids to text using the tokenizer attached to global args."""
    if _is_token_id_list(prompt):
        tok = getattr(args, "_internal_tokenizer", None)
        return tok.decode(prompt) if tok is not None else ""
    return prompt
...
async def async_request_openai_chat_completions(
    ...
    # Determine if prompt is token ids; if so, decode to text for OpenAI chat payloads.
    use_token_ids = _is_token_id_list(request_func_input.prompt)

    if request_func_input.image_data and use_token_ids:
        # For simplicity, we do not support images + token ids combination here.
        # Decode ids to text and still send images via messages.
        decoded_text = _decode_ids_if_needed(request_func_input.prompt)
        content_items = [
            {
                "type": "image_url",
        ...
        content_items.append({"type": "text", "text": decoded_text})
        messages = [
            {
                "role": "user",
        ...
        user_text = _decode_ids_if_needed(request_func_input.prompt) if use_token_ids else request_func_input.prompt
...
ASYNC_REQUEST_FUNCS = {
    ...
    "dynamo-oai-chat": async_request_openai_chat_completions,
...
class BenchmarkMetrics:
    ...
    concurrency: float
    output_throughput_denominator: str
    output_duration_s_for_throughput: float
...
def _median(xs):  return float(np.median(xs)) if xs else 0.0
def _std(xs):     return float(np.std(xs)) if xs else 0.0
def _p(xs, q):    return float(np.percentile(xs, q)) if xs else 0.0
def _max(xs):     return float(np.max(xs)) if xs else 0.0
def _sum(xs):     return float(np.sum(xs)) if xs else 0.0
...
def calculate_metrics(
    ...
    sum_latency = _sum(e2e_latencies)
    sum_ttft = _sum(ttfts)
    if throughput_denom == "decode" and sum_latency > 1e-12 and (sum_latency - sum_ttft) > 1e-12 and dur_s > 0:
        output_denom_s = dur_s * ((sum_latency - sum_ttft) / sum_latency)
    else:
        output_denom_s = dur_s
        throughput_denom = "total"
    ...
    metrics = BenchmarkMetrics(
        ...
        output_throughput=(sum(output_lens) / output_denom_s) if output_denom_s > 0 else 0.0,
        output_throughput_retokenized=(sum(retokenized_output_lens) / output_denom_s) if output_denom_s > 0 else 0.0,
        ...
        p99_e2e_latency_ms=_p(e2e_latencies, 99) * 1000,

        concurrency=_sum(e2e_latencies) / dur_s if dur_s > 0 else 0.0,
        output_throughput_denominator=throughput_denom,
        output_duration_s_for_throughput=output_denom_s,
    )
...
async def benchmark(
    ...
    metrics, output_lens = calculate_metrics(
        ...
        tokenizer=tokenizer,
        backend=backend,
        throughput_denom=args.throughput_denominator,
    )
    ...
    print("{:<40} {:<10}".format("Token throughput denom (output):", metrics.output_throughput_denominator))
    if metrics.output_throughput_denominator == "decode":
        print("{:<40} {:<10.2f}".format("Effective decode duration (s):", metrics.output_duration_s_for_throughput))
    ...
    if (
        ...
        result = {
            ...
            "output_throughput_denominator": metrics.output_throughput_denominator,
            "effective_decode_duration_s": metrics.output_duration_s_for_throughput,
        }
    else:
...
def run_benchmark(args_: argparse.Namespace):
    ...
    if getattr(args, "tokenize_prompt", False):
        assert args.backend in [
            ...
            "`--tokenize-prompt` is only compatible with backends: "
            "`sglang`, `sglang-native`, `sglang-oai-chat`, and `dynamo-oai-chat` currently"
        )
    ...
    elif args.backend in ["sglang-oai-chat", "vllm-chat", "lmdeploy-chat", "dynamo-oai-chat"]:
        api_url = (
    ...
    setattr(args, "_internal_tokenizer", tokenizer)
    input_requests = get_dataset(args, tokenizer)



    if not input_requests:
        raise ValueError("No input requests generated. Check dataset_name/length filters.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        choices=["total", "decode"],
        default="total",
        help=(
            "Denominator used for *output* token throughput. "
            "'total' uses benchmark wall-clock duration; "
            "'decode' excludes TTFT via a concurrency-adjusted estimate."
        ),
    )
