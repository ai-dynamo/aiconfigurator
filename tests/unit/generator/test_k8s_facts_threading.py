...
``build_dgd`` accepts ``resolved_facts`` (default ``None``)
...
def test_end_to_end_render_with_facts_threaded_is_unchanged():
    ...
    a = generate_backend_artifacts(copy.deepcopy(case.params), case.backend, backend_version=case.backend_version)
    b = generate_backend_artifacts(copy.deepcopy(case.params), case.backend, backend_version=case.backend_version)
