...
- LAYERING: the MoE block builder (``models/blocks/moe.py``) is the ONE place
  ...
  may import ``operations.moe_comm`` or construct ``MoEAllToAll`` / ``EPMoE``
...
LARGE_EP_OP_NAMES = frozenset({"MoEAllToAll", "EPMoE"})
...
def _large_ep_constructor_offenders(rel_path: Path, source: str) -> list[str]:
    """Call nodes whose callee names ``MoEAllToAll`` / ``EPMoE`` -- bare
...
def test_blocks_moe_is_the_large_ep_emission_site() -> None:
    ...
    assert len(_large_ep_constructor_offenders(rel, source)) >= 2  # MoEAllToAll + EPMoE
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Path("memory.py"), "from ..operations.moe_comm import EPMoE\n"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Path("memory.py"), "op = EPMoE('n', 1.0)\n"),
        (Path("memory.py"), "op = ops.MoEAllToAll('n', 1.0)\n"),
        (Path("memory.py"), "op = operations.moe_comm.EPMoE('n', 1.0)\n"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Path("memory.py"), "# EPMoE mentioned in a comment\npass\n"),
