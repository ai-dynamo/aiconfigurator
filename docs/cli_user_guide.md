...
The SLA, precision, and speculative-decoding flags (`--strict-sla`, `--request-latency`, `--inclusive-tpot`, the percentile SLA family `--ttft-percentile` / `--tpot-percentile` / `--itl` / `--itl-percentile` / `--request-latency-percentile` / `--sla-refine`, `--nextn`, `--nextn-accepted`, `--database-mode`) have dedicated subsections below. Shared flags such as `--save-dir`, `--top-n`, and `--systems-paths` are described in [Common Arguments](#common-arguments-all-modes).
