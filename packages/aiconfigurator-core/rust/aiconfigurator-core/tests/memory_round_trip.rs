...
//! `aiconfigurator_core.sdk.memory.estimate_kv_cache` (which owns the budget math AND
...
//! import `aiconfigurator_core.sdk.memory` (which imports the maturin-built
...
/// `aiconfigurator_core.sdk.memory` (which transitively imports the maturin-built
/// `aiconfigurator_core`).
fn python_memory_importable() -> bool {
    Python::with_gil(|py| match py.import("aiconfigurator_core.sdk.memory") {
        Ok(_) => true,
        Err(e) => {
            let exe: String = py
                .import("sys")
    ...
    if !python_memory_importable() {
        assert!(
            ...
            "memory_round_trip: AIC_REQUIRE_EMBEDDED_ROUND_TRIP is set but \
             `aiconfigurator_core.sdk.memory` is not importable — run after \
             `maturin develop` with PYTHONPATH including the venv site-packages + src."
        );
        eprintln!(
            "memory_round_trip: SKIP — `aiconfigurator_core.sdk.memory` not importable. \
