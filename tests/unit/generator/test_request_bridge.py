...
"""module_bridge.task_config_to_request emits a GeneratorRequest that lowers
...
def _fake_task_config(mode="agg"):
    # Mirror main's sdk.task_v2.Task surface that module_bridge reads: SLA fields
    # (isl/osl/ttft/tpot/prefix), model flags, and the primary_* properties.
    return SimpleNamespace(
        ...
        primary_backend_version="1.2.0rc5",
        backend_name="trtllm",
...
def _row(prefix=""):
    return pd.Series(
        {
            f"{prefix}{k}": v
            for k, v in {"workers": 1, "tp": 1, "pp": 1, "dp": 1, "moe_tp": 1, "moe_ep": 1, "bs": 128}.items()
        }
    )
...
def test_bridge_injects_encode_role_from_overrides():
    ...
    row = pd.Series(
        ...
        | {
            f"(d){k}": v
            for k, v in {"workers": 1, "tp": 2, "pp": 1, "dp": 1, "moe_tp": 1, "moe_ep": 1, "bs": 128}.items()
        }
    )
