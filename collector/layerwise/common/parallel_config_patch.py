...
def patch_for_parallelism(
    ...
    *,
    ...
    uses_intermediate_as_moe = (
        is_moe and config.get("model_type") == "gpt_oss" and (config.get("moe_intermediate_size", 0) or 0) <= 0
    )
    ...
    if orig_heads % attn_tp != 0:
        raise ValueError(f"num_attention_heads={orig_heads} not divisible by attn_tp={attn_tp}")
    ...
    for linear_head_key in ("linear_num_key_heads", "linear_num_value_heads"):
        ...
        if orig_linear_heads > 0:
            if orig_linear_heads % attn_tp != 0:
                raise ValueError(f"{linear_head_key}={orig_linear_heads} not divisible by attn_tp={attn_tp}")
    ...
    for expert_key in EXPERT_COUNT_KEYS:
        ...
        if orig_experts > 0 and ep > 1:
            ...
            if source_experts % ep != 0:
                raise ValueError(f"{expert_key} source={source_experts} not divisible by ep={ep}")
    ...
    if moe_tp > 1 or uses_intermediate_as_moe:
        ...
        if orig_moe_inter <= 0:
            raise ValueError(f"moe_tp={moe_tp} but config has no {moe_inter_key}")
        if orig_moe_inter % moe_tp != 0:
            raise ValueError(f"{moe_inter_key}={orig_moe_inter} not divisible by moe_tp={moe_tp}")
