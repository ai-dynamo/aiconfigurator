...
class ParallelConfigProjector:
    ...
    def project(self, params: dict[str, Any], backend: str) -> ParallelProjection:
        ...
        if not candidates:
            raise ValueError(f"backend {backend!r} has no valid parallel config in this branch")

        categorical_names = [parameter.name for parameter in self.parameters if parameter.kind == "categorical"]
        ...
        numeric_parameters = [parameter for parameter in self.parameters if parameter.kind != "categorical"]
        backend_features = [
        ...
        selected = min(candidates, key=lambda config: (numeric_distance(config), _config_key(config)))
