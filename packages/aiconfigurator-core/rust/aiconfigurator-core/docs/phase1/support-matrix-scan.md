...
Single SQLite file: `tests/integration/cross_package/scan.sqlite`.
...
uv run python tools/support_matrix/scan_rust_parity.py \
    ...
    --db-path     tests/integration/cross_package/scan.sqlite \
    --workers     16 \
    --scan-mode   both \
    --probe-rtol  0.01 \
    --probe-atol  0.001 \
...
- Benchmark snapshots: `tests/integration/cross_package/benchmarks.md` ("After fix campaign C1+C2+C4+C4-residual (2026-06-01)" section).
