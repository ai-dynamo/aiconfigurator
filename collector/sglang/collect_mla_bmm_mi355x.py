...
def _mla_bmm_dtypes_for_backend() -> list[str]:
...
def run_mla_gen_pre(num_tokens, num_heads, dtype, num_warmups, num_runs, perf_filename, device="cuda:0"):
    ...
    if dtype == "fp8":
        if is_hip() or _runtime_hip_available():
            raise ValueError("mla_bmm FP8 on ROCm/HIP is not supported in this collector; use float16")
        if not _fp8_bmm_available():
    ...
    log_perf(
        ...
        op_name="mla_gen_pre",
        kernel_source="torch_bmm" if dtype == "float16" else "sgl_bmm_fp8",
