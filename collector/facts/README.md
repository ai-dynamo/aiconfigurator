...
| `matrix.yaml` | THE consolidated result (`gen_facts.py --matrix`): per (checkpoint × backend) — verdict (pass / pass+custom / fail), the extra generate args when needed, failure cause, and the deployed identity actually measured (attention backend, MoE quant→kernel family, allocated KV dtype, topology). |
