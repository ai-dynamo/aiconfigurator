...
- `--engine-step-backend`: Engine-step latency backend. The compiled Rust engine is the only step executor; `rust` is the only live value. `python` is DEPRECATED and now a no-op — it warns once and runs on the compiled engine anyway (accepted for one release cycle, then removed). Accepted by all modes but inert in `generate`, which performs no latency estimation. (`default`, `exp`, `generate`, `estimate`)
