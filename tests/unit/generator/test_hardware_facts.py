...
def test_every_profile_has_tolerations_key():
    ...
    for name, p in _profiles().items():
        ...
        assert isinstance(p["tolerations"], list), (
            f"{name}.tolerations must be a list, got {type(p['tolerations'])}"
        )
...
def test_gb200_tolerations_include_gpu():
    ...
    assert "nvidia.com/gpu" in gpu_keys, (
        "gb200 tolerations must include nvidia.com/gpu"
    )
...
def test_hopper_h100_tolerations_empty():
    ...
    """
    h100 = _profiles()["h100"]
    assert h100["tolerations"] == [], (
        "h100 tolerations must be [] — no Hopper H100 recipe shows node taints"
    )
