...
AGG_CFG = ReplicaParallelConfig(shape=ParallelShape(tp=4, dp=1, moe_tp=1, moe_ep=4), replicas=2)
...
def test_agg_parallel_unrolls_to_flat_shape():
    s = unroll_sample(search_space=_space(), selection=_agg_selection(), parallel_config=AGG_CFG)
    assert (s["tp"], s["attention_dp"], s["moe_tp"], s["moe_ep"], s["pp"]) == (4, 1, 1, 4, 1)
    assert s["replicas"] == 2
...
def test_disagg_parallel_unrolls_prefill_and_decode():
    ...
    assert (s["decode_attention_dp"], s["decode_moe_ep"], s["decode_replicas"]) == (8, 8, 2)
...
def test_fpm_and_sensitivity_numeric_expansion():
    sel = _agg_selection(
        planner_scaling_policy="hybrid_180_5", planner_fpm_sampling="large", planner_load_sensitivity="aggressive"
    )
    s = unroll_sample(search_space=_space(), selection=sel, parallel_config=AGG_CFG)
...
def test_load_predictor_resolved_from_sweep_winner():
    lp = LoadPredictorResult(best_by_interval={180: "prophet_w20_log1p", 600: "constant_last"}, reason="swept")
    sel = _agg_selection(planner_scaling_policy="throughput_180_5")
...
def test_load_predictor_omitted_without_throughput_scaling():
    ...
    sel = _agg_selection(planner_scaling_policy="load_180_5")  # load-only -> no forecaster
    s = unroll_sample(search_space=_space(), selection=sel, parallel_config=AGG_CFG, load_predictor=lp)
...
def test_router_knobs_gated_by_router_mode():
    s = unroll_sample(
        search_space=_space(), selection=_agg_selection(router_mode="round_robin"), parallel_config=AGG_CFG
    )
