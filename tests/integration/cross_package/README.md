# Cross-Package Rust/Python Parity Tests

Temporary harness for the Rust `aiconfigurator-core` migration. (To be deprecated after the transition)

This directory contains core compile parity checks, parity tests that exercise
the upper CLI against the Rust-backed core, and the Rust-Python speed benchmark.
...
```bash
AICONFIGURATOR_RUST_CORE_AUTOBUILD=1 uv run pytest -q -rx tests/integration/cross_package/test_engine_step_parity.py
```
...
```bash
python tests/integration/cross_package/benchmark_engine_step.py --warmup 5 --iterations 50
```
...
```bash
python tests/integration/cross_package/benchmark_engine_step.py --case minimax-m25 --warmup 5 --iterations 50
python tests/integration/cross_package/benchmark_engine_step.py --case kimi-k25 --warmup 10 --iterations 100
python tests/integration/cross_package/benchmark_engine_step.py --case kimi-k25 --cache-mode cold --warmup 0 --iterations 50
python tests/integration/cross_package/benchmark_engine_step.py --case minimax-m25 --json
```
