...
| 120 | **FLASHINFER_CUTLASS** | `FlashInferExperts._supports_current_device` includes `is_device_capability_family(120)` (`experts/flashinfer_cutlass_moe.py:129-140`); its `_supports_quant_scheme` for `(None,None)` needs only `has_device_capability(90)` — a **floor**, true at 120 too (`platforms/interface.py:149-165/433-454`) |
...
Which candidate actually wins also depends on `validate_configuration` (`v1/attention/backend.py:319-374`) rejecting invalid candidates, and this is exactly where SM90 and SM120 diverge on fp8 KV cache:
...
4. **Fp8_block SM100/103's `_supports_router_logits_dtype` gate** (item 1) — flagged, not fully closed; does not change the resolved-backend conclusion (FLASHINFER_TRTLLM is still first-priority and the only other viable candidate paths don't apply here), but a future verification pass could nail down the exact default-resolution site.
