...
# Experiment — router config sweep to minimize e2e latency (MiniMax-M2.5 / B200)
...
# search (Vizier) on the 2k subset, then validate the winner on the full trace:
python -m spica --config <this-config>.yaml            # search
# validate: run the winning router config via dynamo.replay.run_trace_replay on the full trace
#   KvRouterConfig(overlap_score_credit=1.0, prefill_load_scale=32.0, router_temperature=0.0)
#   num_workers=8, replay_concurrency=32, replay_mode="offline", trace_block_size=512
```

Notes: the AIC perf model needs the `aic-forward-pass` binding (see README); suppress router
logs with `RUST_LOG=error`. `prefill_load_scale` up to 32 requires the extended
`SEARCH_CHOICES` in `config.py`.
