...
The large-EP redesign's operators are native. `MoeAllToAll` (spec variant 31)
executes over measured `moe_a2a_perf`. The schema-v7 wire name `EpMoe`
(variant 32) executes `ModeledEpMoeOp`, which estimates balanced local expert
compute from stock `moe_perf`; there is no dedicated EP-compute table or
loader. The spec emitter converts these ops natively.
