...
flowchart LR
    ...
    DATA["AIC perf DB<br/>src/aiconfigurator_core/systems/data"]
...
1. Keep Rust/Python comparison assets under
   `tests/integration/cross_package/` and run them explicitly until Python
   SDK deprecation.
...
- [x] Rust/Python parity smoke tests exist under
  `tests/integration/cross_package/`.
...
- [x] Final Phase 3 benchmark snapshot recorded in `tests/integration/cross_package/benchmarks.md`.
...
```bash
AICONFIGURATOR_RUST_CORE_AUTOBUILD=1 pytest tests/integration/cross_package/test_engine_step_parity.py
```
