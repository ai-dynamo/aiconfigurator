...
if hasattr(torch, "float8_e4m3fnuz"):
    _FP8_KV_DTYPES = _FP8_KV_DTYPES + (torch.float8_e4m3fnuz,)
...
class MockServerArgs:
    def __init__(self, kv_cache_dtype: torch.dtype, page_size: int):
        ...
        self.kv_cache_dtype = "fp8" if kv_cache_dtype in _FP8_KV_DTYPES else "float16"
...
def benchmark_layer(
    ...
    **kwargs,
    ...
    def kernel_func():
        if use_fused_mla and q_rope is not None and k_rope is not None:
            q_in = torch.cat([q, q_rope], dim=-1)
            k_in = torch.cat([k, k_rope], dim=-1)
            layer(q_in, k_in, v, forward_batch, **kwargs)
        else:
    ...
    with benchmark_with_power(
        ...
        num_runs=20,
        repeat_n=1,
        allow_graph_fail=is_hip() or _runtime_hip_available(),
    ) as results:
        pass
...
def run_mla(
    ...
    if is_context_phase:
        ...
        if kernel_source == "aiter_attention":
            positions = torch.cat(
                [torch.arange(1, input_len, device=torch_device) for _ in range(batch_size)]
            )
            out_cache_loc = token_matrix[:, 1:input_len].reshape(-1).to(torch.int32)
            input_ids_ctx = torch.zeros(batch_size, eff_tokens, dtype=torch.long, device=torch_device)
        else:
            ...
            input_ids_ctx = torch.zeros(batch_size, input_len, dtype=torch.long, device=torch_device)
        ...
        if kernel_source == "aiter_attention":
            prefix_loc = token_matrix[:, 0].reshape(-1).contiguous()
            cache_k_p = torch.randn(
                prefix_loc.numel(),
                1,
                kv_lora_rank,
                device=torch_device,
