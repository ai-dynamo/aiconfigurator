...
def _create_non_trtllm_attn_backend(model_runner):
    ...
    Returns:
        (attn_backend, kernel_source_tag) for CSV logging (e.g. ``aiter_attention``, ``triton_attention``).
    """
    raw = (os.environ.get("SGLANG_COLLECT_ATTN_BACKEND") or "auto").strip().lower()
    if raw in ("", "auto"):
        choice = "aiter" if is_hip() else "triton"
    else:
        choice = raw
    ...
    if choice == "triton":
        try:
            ...
            return TritonAttnBackend(model_runner), "triton_attention"
        except Exception as e:
            logger.warning("TritonAttnBackend unavailable (%s); falling back to AiterAttnBackend", e)
            from sglang.srt.layers.attention.aiter_backend import AiterAttnBackend

            return AiterAttnBackend(model_runner), "aiter_attention"
    ...
    raise ValueError(
        f"Invalid SGLANG_COLLECT_ATTN_BACKEND={raw!r}; use 'auto', 'aiter', or 'triton'"
    )


def _get_effective_sm_version():
    """SM version for feature checks. On ROCm/HIP return 80 to skip FP8 and TRTLLM."""
    if is_hip():
        return 80
...
def get_context_attention_test_cases():
    ...
    # FP8 attention requires SM90+ (Hopper); on ROCm use effective SM 80 to skip FP8
...
def run_attention_torch(
    ...
    *,
    ...
    warmup = 3
    # Use benchmark_with_power context manager; on ROCm allow graph capture to fail and use eager
