...
class TestEncoderMemoryInSummary:
    ...
    def test_vl_model_without_image_dimensions_skips_encoder_queries(self, model_config):
        """Default text-only VL requests must not query zero-token encoder GEMMs."""
        from types import SimpleNamespace
        from unittest.mock import MagicMock
        ...
        for op in model.encoder_ops:
            op.query = MagicMock(side_effect=AssertionError("encoder should be skipped"))
        ...
        assert summary.get_encoder_memory() == {}
        assert all(op.query.call_count == 0 for op in model.encoder_ops)
