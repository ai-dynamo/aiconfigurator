...
class TestEncoderMemoryInSummary:
    ...
    def test_vl_model_without_image_shape_skips_encoder(self, model_config):
        """VL model defaults can be used for text-only requests without zero-token encoder queries."""
        from types import SimpleNamespace
        from unittest.mock import MagicMock
        ...
        for op in model.context_ops + model.generation_ops:
            op.query = MagicMock(return_value=MagicMock(__float__=lambda s: 1.0, energy=0.0, source="silicon"))
        for op in model.encoder_ops:
            op.query = MagicMock(side_effect=AssertionError("text-only request should not query encoder ops"))
