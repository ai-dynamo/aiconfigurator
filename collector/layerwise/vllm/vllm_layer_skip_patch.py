...
def _make_moe_noop_forward():
    def _moe_noop_forward(self, *args, **kwargs):
        ...
        if hidden_states is None:
            raise RuntimeError(
                f"LAYERWISE_MOE_NOOP could not find hidden_states tensor for {type(self).__name__}.forward"
            )
        return hidden_states
...
def _apply_moe_noop(model, target_layers: set[int]) -> int:
    ...
    if layers is None:
        logger.warning("[vllm-layer-skip] LAYERWISE_MOE_NOOP requested but could not locate transformer layers")
        return 0
    ...
    if missing:
        logger.warning(
            f"[vllm-layer-skip] LAYERWISE_MOE_NOOP found no MoE MLP in target layer(s) {missing} under {path}"
        )
    logger.warning(f"[vllm-layer-skip] LAYERWISE_MOE_NOOP patched={patched}")
    return len(patched)
...
def _apply_router_weights(model, source_model: str, target_layers: set[int]) -> int:
    ...
    for module_name, module in model.named_modules():
        ...
        source_layer = int(sorted_targets[layer_position]) if layer_position < len(sorted_targets) else layer_position
...
def _install_patch():
    ...
    def patched(self, *args, **kwargs):
        ...
        try:
            ...
            if compile_mode == CompilationMode.NONE and getattr(
                self.vllm_config.observability_config, "enable_layerwise_nvtx_tracing", False
            ):
                from vllm.utils.nvtx_pytorch_hooks import PytHooks

                _pyt_hooks = PytHooks()
                _pyt_hooks.register_hooks(self.model, self.model.__class__.__name__)
                self.layerwise_nvtx_hooks_registered = True
                logger.warning("[vllm-layer-skip] registered PytHooks early (compile=NONE path, before graph capture)")
        except Exception:
    ...
    logger.warning(f"[vllm-layer-skip] installed GPUModelRunner.load_model patch, targets={sorted(targets)}")
