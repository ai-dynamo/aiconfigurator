...
# Collector sources intentionally live in the upper AIC project but are not
# part of its runtime wheel. Make that project root importable for collector
# unit tests without shadowing either installed Python distribution.
_WORKSPACE_ROOT = Path(__file__).resolve().parents[1]
_AIC_PROJECT_ROOT = _WORKSPACE_ROOT / "packages" / "aiconfigurator"
if _AIC_PROJECT_ROOT.is_dir() and str(_AIC_PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(_AIC_PROJECT_ROOT))
