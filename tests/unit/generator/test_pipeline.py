...
def test_pipeline_artifacts_match_legacy(case):
    ...
    legacy = generate_backend_artifacts(copy.deepcopy(case.params), case.backend, backend_version=case.backend_version)
    result = run_pipeline(copy.deepcopy(case.params), case.backend, backend_version=case.backend_version)
