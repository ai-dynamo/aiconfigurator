...
class TestHeteroTimesSupernode:
    ...
    def test_tier_selection_differs_per_side_at_the_same_span(self):
        """A=rack-aware and F=flat resolve span 144 to different bandwidths."""
        ...
        assert _tiered_latency(_FLAT_NODE_SPEC, 1 << 20, 144) > _tiered_latency(_RACK_NODE_SPEC, 1 << 20, 144)
...
class TestEngineHonorsTheSpan:
    ...
    def test_crossing_the_rack_repricing_matches_the_spec(self, rack_database):
        ...
        latency = node.get("inter_rack_latency", node["p2p_latency"])
