...
# Experiment — planner config sweep to maximize goodput-per-GPU (MiniMax-M2.5 / B200)
...
python -m spica --config <this-config>.yaml      # full-trace, open-loop planner sweep
```
...
[router-e2e-latency-sweep.md](router-e2e-latency-sweep.md).
