...
def test_missing_model_path_is_error():
    r = GeneratorRequest(model=ModelSpec(model_path=""), backend=BackendSpec(name="trtllm"),
                         topology=Topology(total_gpus=8))
...
def test_mode_role_consistency():
    ...
    # valid disagg
    r3 = _req(mode="disagg", roles={"prefill": RoleSizing(tensor_parallel_size=1),
                                    "decode": RoleSizing(tensor_parallel_size=1)})
    assert r3.validate() == []


def test_role_sizing_roundtrips_params():
    params = {"tensor_parallel_size": 4, "pipeline_parallel_size": 1, "gpus_per_worker": 4,
              "max_batch_size": 128, "kv_cache_dtype": "fp8"}
