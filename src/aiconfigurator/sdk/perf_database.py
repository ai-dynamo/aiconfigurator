...
from aiconfigurator.sdk.interpolation import (
    bilinear_interpolation as _bilinear_interpolation_fn,
)
from aiconfigurator.sdk.interpolation import (
    estimate_mem_op as _estimate_mem_op_fn,
)
from aiconfigurator.sdk.interpolation import (
    extract_latency_and_energy_2d as _extract_latency_and_energy_2d_fn,
)
from aiconfigurator.sdk.interpolation import (
    extract_latency_and_energy_3d as _extract_latency_and_energy_3d_fn,
)
from aiconfigurator.sdk.interpolation import (
    extrapolate_data_grid as _extrapolate_data_grid_fn,
)
from aiconfigurator.sdk.interpolation import (
    get_sample_leaf_value as _get_sample_leaf_value_fn,
)
from aiconfigurator.sdk.interpolation import (
    get_value as _get_value_fn,
)
from aiconfigurator.sdk.interpolation import (
    interp_1d as _interp_1d_fn,
)
from aiconfigurator.sdk.interpolation import (
    interp_2d_1d as _interp_2d_1d_fn,
)
from aiconfigurator.sdk.interpolation import (
    interp_2d_linear as _interp_2d_linear_fn,
)
from aiconfigurator.sdk.interpolation import (
    interp_3d as _interp_3d_fn,
)
from aiconfigurator.sdk.interpolation import (
    interp_3d_linear as _interp_3d_linear_fn,
)
from aiconfigurator.sdk.interpolation import (
    nearest_1d_point_helper as _nearest_1d_point_helper_fn,
)
from aiconfigurator.sdk.interpolation import (
    validate_interpolation_result as _validate_fn,
)
...
class PerfDatabase:
    ...
    def __init__(self, system: str, backend: str, version: str, systems_root: str = "./systems") -> None:
        ...
        self.system_spec = SystemSpec.from_yaml_dict(raw_spec)
    ...
    def _get_value(self, data_value, metric: str = "latency"):
        """Extract a metric from a data value. Delegates to interpolation module."""
        return _get_value_fn(data_value, metric)
    ...
    def _extract_latency_and_energy_2d(self, data: dict) -> tuple[dict, dict]:
        """Extract latency and energy from 2D data. Delegates to interpolation module."""
        return _extract_latency_and_energy_2d_fn(data)

    def _extract_latency_and_energy_3d(self, data: dict) -> tuple[dict, dict]:
        """Extract latency and energy from 3D data. Delegates to interpolation module."""
        return _extract_latency_and_energy_3d_fn(data)
    ...
    def _extrapolate_data_grid(
        ...
        """Extrapolate the data grid. Delegates to interpolation module."""
        _extrapolate_data_grid_fn(data_dict, target_x_list, target_y_list, target_z_list, sqrt_y_value)

    def _nearest_1d_point_helper(self, x: int, values: list[int], inner_only: bool = True) -> tuple[int, int]:
        """Find the nearest 1d point. Delegates to interpolation module."""
        return _nearest_1d_point_helper_fn(x, values, inner_only)

    def _validate(self, value: float) -> float:
        """Validate the value. Delegates to interpolation module."""
        return _validate_fn(value)

    def _interp_3d_linear(self, x: int, y: int, z: int, data: dict) -> float:
        """Interpolate 3D data using linear interpolation. Delegates to interpolation module."""
        return _interp_3d_linear_fn(x, y, z, data)

    def _interp_2d_linear(self, x: int, y: int, data: dict) -> dict:
        """Interpolate 2D data using linear interpolation. Delegates to interpolation module."""
        return _interp_2d_linear_fn(x, y, data, self._extracted_metrics_cache)

    def _interp_3d(self, x: int, y: int, z: int, data: dict, method: str) -> dict:
        """Interpolate 3D data using the given method. Delegates to interpolation module."""
        return _interp_3d_fn(x, y, z, data, method, self._extracted_metrics_cache)

    def _get_sample_leaf_value(self, data: dict):
        """Get a sample leaf value from nested dict. Delegates to interpolation module."""
        return _get_sample_leaf_value_fn(data)

    def _get_p2p_bandwidth(self, num_gpus: int) -> float:
        """Get P2P bandwidth based on GPU count. Delegates to SystemSpec."""
    ...
    def _bilinear_interpolation(self, x_list: list[int], y_list: list[int], x: int, y: int, data: dict) -> float:
        """Bilinear interpolation. Delegates to interpolation module."""
        return _bilinear_interpolation_fn(x_list, y_list, x, y, data)

    def _interp_2d_1d(self, x: int, y: int, z: int, data: dict, method="bilinear") -> float:
        """Interpolate 3D data using 2D-then-1D method. Delegates to interpolation module."""
        return _interp_2d_1d_fn(x, y, z, data, method)

    def _interp_1d(self, x: list[int], y: list, value: int):
        """Interpolate 1D data using linear interpolation. Delegates to interpolation module."""
        return _interp_1d_fn(x, y, value)
    ...
    def query_mem_op(
        ...
        """Query memory operation latency and energy. Delegates to interpolation module."""
        return _estimate_mem_op_fn(
            mem_bytes=mem_bytes,
            mem_bw=self.system_spec["gpu"]["mem_bw"],
            mem_bw_empirical_scaling_factor=self.system_spec["gpu"].get("mem_bw_empirical_scaling_factor", 1.0),
            mem_empirical_constant_latency=self.system_spec["gpu"].get("mem_empirical_constant_latency", 0.0),
            database_mode=database_mode,
