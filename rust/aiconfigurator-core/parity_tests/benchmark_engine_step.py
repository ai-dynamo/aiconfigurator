...
- Rust estimator setup: timed separately from step latency. Includes the
  PyO3 extension import, Rust model metadata load, Rust perf DB load, and
...
The Rust core is consumed as the installed `aiconfigurator-core` PyO3 wheel;
install it with `pip install -e ".[rust]"` or `maturin develop --release`.
"""
...
def _ensure_rust_library_present() -> None:
    if not rust_engine_step.is_rust_core_available():
        raise RuntimeError(
            "Rust core PyO3 extension is not importable. Install it with "
            '`pip install -e ".[rust]"` or `maturin develop --release`.'
        )
