...
"""Apply model-profile ``defaults:`` to rendered cli token lists (Phase 4a).
...
def _entry_matches(match: dict[str, Any], *, backend: str, system: str | None, variant: str | None) -> bool:
    ...
    / ``model_variant`` must equal the request's value. Ported from
    ``defaults_pass._entry_matches`` (which compared against the IR fields).
    """
    if match.get("backend") and match["backend"] != backend:
        return False
    if match.get("system") and match["system"] != (system or ""):
        return False
    if match.get("model_variant") and match["model_variant"] != (variant or ""):
        return False
    return True
...
def _stringify(value: Any) -> str:
    ...
    compact JSON, matching ``legacy_adapter._cli_tokens_from_args``.
    """
...
def apply_facts(context: dict[str, Any], facts: "ResolvedFacts | None", backend: str) -> None:
    ...
    # Phase 4a: model_variant is not yet resolved per request.
    ...
    for role in ("prefill", "decode", "agg"):
        ...
        apply_model_default_args(
            tokens, facts.model, backend=backend, system=system, role=role, variant=variant,
        )
