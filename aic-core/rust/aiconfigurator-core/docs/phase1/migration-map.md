...
flowchart LR
    ...
    FFI["Rust C ABI<br/>aic-core/rust/aiconfigurator-core/src/ffi.rs"]
...
1. Keep Rust/Python comparison assets under
   `aic-core/rust/aiconfigurator-core/parity_tests/` and run them explicitly until Python
   SDK deprecation.
...
- [x] Rust/Python parity smoke tests exist under
  `aic-core/rust/aiconfigurator-core/parity_tests/`.
...
```bash
AICONFIGURATOR_RUST_CORE_AUTOBUILD=1 pytest aic-core/rust/aiconfigurator-core/parity_tests/test_engine_step_parity.py
```
