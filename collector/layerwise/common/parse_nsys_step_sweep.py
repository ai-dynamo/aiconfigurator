        ...
        if key not in kern_meta:
            ...
            if not dropped:
                step = _step_of_with_starts(
                    step_wins.get(tid, []), step_starts.get(tid, []), rt_start
                )
                if step is None:
                    step = _step_of_with_starts(
                ...
                if step is None:
                    step = _step_of_with_starts(
                        all_step_wins, all_step_starts, rt_start
                    )
                    if step is not None:
...
def parse_step_sweep(
    ...
    *,
    ...
    if os.path.getsize(sqlite_path) == 0:
        raise RuntimeError(
            f"nsys sqlite export is empty: {sqlite_path}. "
            "Re-export the report with an explicit `--output` path."
        )
...
def main():
    ...
    print(f"{'step':>8} {'bs':>5} {'past_kv':>8} {'run':>5}  {rank_col}"
    ...
    for step_meta in step_seen:
        for rank in rank_seen:
            ...
            print(f"{step_n:>8} {bs:>5} {past:>8} {measure_run:>5}  {rank_prefix}"
                  + "  ".join(row_cells))
