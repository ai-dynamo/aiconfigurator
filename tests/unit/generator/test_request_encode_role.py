...
"""EPD: the optional `encode` role flows through the data layer.
...
def test_collect_without_encode_has_no_encode_keys():
    params = collect_generator_params(
        service={"model_path": "m"},
        k8s={},
        agg_params={"tensor_parallel_size": 1},
        agg_workers=2,
        backend="trtllm",
    )
...
def test_collect_with_encode_adds_role_and_worker_count():
    params = collect_generator_params(
        service={"model_path": "m"},
        k8s={},
        prefill_params={"tensor_parallel_size": 4},
        decode_params={"tensor_parallel_size": 4},
        prefill_workers=1,
        decode_workers=1,
        encode_params={"tensor_parallel_size": 1, "gpus_per_worker": 1},
        encode_workers=1,
        backend="trtllm",
    )
...
def test_encode_role_allowed_in_agg_mode():
    ...
    req = GeneratorRequest(
        model=ModelSpec(model_path="m"),
        backend=BackendSpec(name="sglang"),
        topology=Topology(
            mode="agg",
            roles={"agg": RoleSizing(tensor_parallel_size=1), "encode": RoleSizing(tensor_parallel_size=1)},
