...
def test_fpm_case_generation_and_shell_command(tmp_path):
    ...
    assert cmd.argv[cmd.argv.index("--real-workload-requests") : cmd.argv.index("--real-workload-requests") + 2] == [
        "--real-workload-requests",
        "128",
    ]
    assert cmd.argv[cmd.argv.index("--real-workload-isl-max") : cmd.argv.index("--real-workload-isl-max") + 2] == [
        "--real-workload-isl-max",
        "16384",
    ]
    assert cmd.argv[cmd.argv.index("--real-workload-osl-mean") : cmd.argv.index("--real-workload-osl-mean") + 2] == [
        "--real-workload-osl-mean",
        "1024",
    ]
    assert cmd.argv[cmd.argv.index("--request-allow-failures") : cmd.argv.index("--request-allow-failures") + 2] == [
        "--request-allow-failures",
        "3",
    ]
    assert cmd.argv[cmd.argv.index("--prompt-token-mode") : cmd.argv.index("--prompt-token-mode") + 2] == [
        "--prompt-token-mode",
        "safe_ascii",
    ]
