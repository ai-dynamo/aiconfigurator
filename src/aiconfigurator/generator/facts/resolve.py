...
"""Pure resolver that loads the Phase 2 facts files into a ResolvedFacts dataclass.

This module is intentionally isolated from the pipeline (not imported by pipeline.py).
Phase 3 will wire it in.
"""
...
def resolve_facts(
    *,
    ...
    try:
        ...
        raise KeyError(
            f"No entry for dynamo_version={dynamo_version!r} backend={backend!r} "
            f"in {runtimes_path}"
        ) from exc
    ...
    if transport not in transport_profiles:
        raise KeyError(
            ...
            f"Available profiles: {sorted(transport_profiles)}"
        )
