...
class DynamoConfig:
    """
    User-provided override bag (flat dict). Also exposes helpers to split
...
class ParallelSpecBase:
    """Common parallel spec shared by Agg and Disagg roles."""
    tp: int
    pp: int
    dp: int
    bs: int
...
class BestAggConfig(ParallelSpecBase):
    """Best Agg config using the common parallel spec base."""

@dataclass
class RoleParallelSpec(ParallelSpecBase):
    """Parallel spec for a Disagg role with worker count."""
    workers: int = 1

@dataclass
class BestDisaggConfig:
    """Best Disagg config with prefill and decode role specs."""
...
class GeneratorContext:
    """
    Canonical, validated view consumed by backend generators.
    """
    model_name: str
    backend: str
    version: str
