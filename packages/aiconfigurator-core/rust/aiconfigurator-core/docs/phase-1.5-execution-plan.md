...
aiconfigurator_core::build_aic_engine(model_path, runtime_config)
       ...
       ├─► Python::with_gil { py.import("aiconfigurator_core.sdk.engine")
...
pub fn build_aic_engine(
    ...
    let spec_bytes: Vec<u8> = Python::with_gil(|py| -> PyResult<Vec<u8>> {
        let aic_core = py.import("aiconfigurator_core.sdk.engine")?;
        aic_core
            .call_method1(
                "compile_engine",
...
`aiconfigurator_core.sdk.memory.estimate_num_gpu_blocks` helper (the same
...
| **E8** | Perf gate | Benchmark `engine.run_static` / `engine.run_agg` against Phase 1's ctypes path on the smoke set. Exit criterion: **≥3× p50 speedup** on small-graph families that today regress below 1× (MiniMax-M2.5 gen, NemotronNas gen, DSv3.2 ctx) by eliminating the per-call Python op-walking. Big-graph families stay at or above Phase 1 speedup. Numbers land in `tests/integration/cross_package/benchmarks.md`. | **GATE** |
...
- FFI-tax breakdown that drives the batched-entry decision:
  `tests/integration/cross_package/benchmarks.md` ("FFI overhead caveat" section).
