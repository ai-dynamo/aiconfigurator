...
def load_mamba2_data(mamba2_file: str):
    ...
    for row in rows:
        ...
        if phase == "context":
            ...
            # Decode is a one-token operation. Its consumer key is batch-only;
            # ``seq_len`` is collector metadata and must not create a second
            # leaf for the same physical lookup point.
