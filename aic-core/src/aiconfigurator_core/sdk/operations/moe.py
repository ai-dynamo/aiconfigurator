...
class ModeledEPMoE(Operation):
    ...
    The approximation assumes uniform routing balance. The Rust engine maps
    global per-rank tokens onto the local expert shard and always reports the
    result as estimated; no retired ``moe_expert_compute`` table is queried.
    """
    ...
    def modeled_coordinates(self, x: int) -> dict[str, int | str]:
        """Return the stock-MoE coordinates represented by the Rust op."""
...
class TrtLLMWideEPMoE(Operation):
    ...
    def _engine_query_plan(self, kwargs: dict):
        raise NotImplementedError(
            "TrtLLMWideEPMoE was retired from the active large-EP path; "
            "build MoEAllToAll + ModeledEPMoE and evaluate that op list instead."
        )
