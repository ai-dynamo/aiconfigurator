...
authored current/previous plus a fleet-derived dev (highest data-backed
version newer than current; override systems are frozen baselines with no
dev). The literal aliases current/previous/dev resolve anywhere a version is
...
def systems_root(tmp_path):
    ...
    # current + previous data on h200; a developer drop at a newer version
    ...
    # a marker-only newer dir must NOT count as dev
    ...
    pdb._derive_fleet_dev.cache_clear()
    yield str(root)
    pdb._load_query_slots_doc.cache_clear()
    pdb._derive_fleet_dev.cache_clear()
...
def test_slots_resolution_and_fleet_dev(systems_root):
    ...
    assert slots == {"current": "1.3.0rc20", "previous": "1.2.0rc5", "dev": "1.3.0rc23"}


def test_marker_only_dirs_do_not_qualify_as_dev(systems_root):
    slots = pdb.get_version_slots("h200_sxm", "trtllm", systems_root)
    assert slots["dev"] != "1.4.0rc1"
...
def test_aliases_resolve(systems_root):
    ...
    assert r("h200_sxm", "trtllm", "dev", systems_root) == "1.3.0rc23"
...
def test_missing_alias_raises_with_slots_listed(systems_root):
    with pytest.raises(ValueError, match="has no 'dev'"):
        pdb.resolve_query_version("a100_sxm", "trtllm", "dev", systems_root)
