...
"""Wiring: api.generate_from_request and request.from_cli."""
...
@pytest.mark.parametrize("name", ["trtllm_dense_agg", "deepseek_trtllm_b200_disagg", "deepseek_sglang_gb200_agg"])
def test_save_results_render_path_is_equivalent(name):
    """The rewired `default`-mode render step (from_legacy_params(cfg) ->
...
def _cli_args(**kw):
    base = dict(
        ...
        system="h200_sxm",
        total_gpus=16,
        generator_dynamo_version="1.1.0",
