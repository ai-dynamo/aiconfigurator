...
> Consolidated on 2026-07-09: process-iteration entries compressed to outcomes; full history in git.
...
Implemented-state and probe outcomes (process compressed 2026-07-09): the
chunked context state makes the independent boundary fail-closed by passing
`allow_graph_fail=False` for every chunk/mode (decode retains its old
single-kernel `allow_graph_fail=True` path and is not relabeled as production
decode), and is frozen read-only at `post_glm_topk_chunk_v1`, manifest
`893724083c1f11a0944cc87d97b98ce5c604f624b1ada2661f75fd310a1658bc`
(GLM collector SHA256 moved from failing `38a95c8a...` to
`77dd7cf361d7da657153dfe0d083922662f849d97e156fee1f173708851d7e4f`). The
bounded H20 oracle's final V3 result `e58ae847...` shows independent-graph
absolute top-last latency is 9.5% above a single-graph chunk loop while the
consumed `flat - top_last` delta differs by only 0.1225%; the absolute
boundary remains disclosed and there is no evidence-based reason to expand
this patch into an MQA rewrite. Seven fresh-container H20 probes passed
(original `(bs=16,isl=1024,prefix=1M)` failure executes 36 chunks; logical
16-TiB `(32,4096,1044480)` executes 588 chunks in 151.9 seconds), and the
later `collect.py` run was row-complete at 11/11 tasks and 6,664/6,664 rows
(CSV `e327b330...`, checkpoint `da98312d...`, canonical key hash
`e714eac7...`; 6,376 ragged context and 288 paged decode rows). Structural
validators `36662f4f...`/`1bd717b1...` PASS but do not audit backend
selection; the completed selector audit semantically rejects the ragged
artifact and its composite (`5fc2ab60...`). Source-file integrity is
separately fail-closed by `sha256sum -c ARTIFACTS.sha256`, which passes in
the composite root.
...
The corrected-candidate chain (`c5cba92d...` context-only, `8f05cbfd...`
omitted decode `row_starts`, `6da3683b...` pad64 decode, frozen `1194e6b6...`
adding chunk-page-table release) is recorded in the Current evidence bundle.
Preserve concatenated context geometry, absolute row starts, bounded Q chunks,
omitted decode optionals, and pad64 decode stride. Acceptance required context
and decode value/leaf oracles, H20 smoke with leaf and peak-memory evidence, a
new read-only snapshot, all 6,664 rows, and a validator that rejects the old
mixed-path artifact; B200 must reverse-validate the same contracts rather than
reinterpret Hopper latency.

Gate outcomes (process compressed 2026-07-09): the H20 selector probe exits 0
with 37/37 observations of FP8 + `flashmla_kv` + EXTEND + `sgl-kernel` =>
PAGED (its first execution is retained as an orchestration failure — missing
task-local perf-output directory); it validates only the SM90 selector, and
Blackwell must rerun the same probe and append its actual backend/transform
result. Bounded value gates pass against frozen collector `1194e6b6...`:
context oracle `4a433d91...` proves unchunked and forced-three-row PAGED
output match a non-identity PyTorch mapped-index reference at `(2,8,4096)`;
decode oracle `20a45ad2...` proves sequence/page width 4097, score stride
4160, omitted `row_starts`, decode leaf present/prefill leaf absent, CUDA
Graph use for both modes, and the mixed `[2048,2049]` boundary. The PAGED
stress runner had a pre-execution reversal (draft `53d00907...` misreported a
materialized chunk page table as `[rows, bs*S]`; the actual table is
`[rows,S]` while only the score is `[rows,bs*S]`); the corrected runner
`5b80b17a...` then passed all eight H20 stress shapes with chunk counts 3--588
and exactly `chunks*2` mandatory graph calls — batch-one ceiling
`(1,16384,1032192)` peaks at 60,567,992,832 allocated bytes in three chunks,
logical 16-TiB `(32,4096,1044480)` completes 588 chunks/1,176 graph calls in
152.734 seconds at 31,023,648,256 peak allocated bytes (identities in
`paged_stress_v1/STRESS_ARTIFACTS.sha256`).

Full-run acceptance: validator `930069f7...` exits 1/FAIL against the rejected
row-complete `89372408...` ragged data (the negative replay that a future
validator must preserve) and 0/PASS against the fresh no-resume
`glm_topk_paged_full_v1` run — 11/11 tasks, 6,664/6,664 rows (3,188 context +
144 decode physical keys, two exact modes each, phase rows 6,376/288, modes
3,332/3,332), all sources `fast_topk_transform_fused`, canonical key hash
`e714eac7...`, new CSV `0755831a...` distinct from rejected ragged CSV
`e327b330...`. Earlier table text saying `1194e6b6...` "remains
GPU-unvalidated" or "remains pending" is superseded, not erased: current SM90
state is selector/value/stress/full complete; only the Blackwell hardware pass
and the other independently tracked operations remain pending. The accepted
full namespace is read-only; `ARTIFACTS.sha256` has SHA256
...
### Proposal and validator revision log (stub, consolidated 2026-07-09)

Superseded proposal/validator iterations; outcomes only. The DSV4 bounded-pool
comparator README went through two rejected pre-GPU drafts; only the final
read-only README
`6c5f1796c521749abd5ae72da3eaa206a6cb7450d7be93b88ba8e4e96891fa09`
(restoring bs512 and bs1024 controls) may drive the future comparator.
The GLM sparse strict validator's old one-gap exception was invalidated;
`dec88c99723c57865c371ef86ec9e174607987a94125a7d0082f57b8b55a6df8` requires
both SM90 and Blackwell at 11/11 tasks and 6,664/6,664 rows, and a future
validator must reject the row-complete mixed-path ragged artifact
(`89372408...`); composite provenance integrity is checked by
`sha256sum -c ARTIFACTS.sha256`, not by the semantic validator. The GLM top-k
chunk proposal wording was corrected pre-implementation to kernel-only
additive calibration (not production sequential latency; power must not be
summed); oracle revisions V1→V3 ended at per-row set equality with both score
modes and all benchmark boundaries. Final outcomes live in the GLM sections
above.
...
### DSV4 live-cap candidate status binding (stub, consolidated 2026-07-09)

Appends to `DSV4-DERIVED-POOL-WORKSPACE`. Chronology correction retained for
replay: `2e197491`, not `299aaea9`, performed the R091 collector-file rename;
`1d12d321` introduced the model YAML without module prefixes. The
live-configurator CSA cap is new exact-0.5.14 behavior (inherited / not
introduced by Blackwell), frozen as `post_dsv4_live_cap_v1` with the accepted
workspace calculation `raw_c4=ceil((prefix+isl)/page)*(page/4)`,
`stride=align_up(raw_c4,256)`, `W=bs*isl*stride*4` (bs8/16 maximum
8,204,058,624 bytes; sgl-deep-gemm 0.1.3 `split_kv=256`). The focused H20 gate
passed all six fresh-container scenarios (row counts `6,6,6,1,1,6`, 26 unique
positive rows, all SM90 DeepGEMM leaves), recovering the nine exact bs8/16/32
failures with controls; frozen evidence manifest SHA256 is
`2803697e0b636c6853af8095f911059c495d15bba44fdbeabf2be014dc2df88a`. Superseded
as present-tense status by the full SM90 stage update and `final_v1`/PR #1302
entries below; SM100/103 remain source-derived and SM120 fail-closed.
...
### DSV4 full SM90 stage update (2026-07-03) (stub, consolidated 2026-07-09)

All four DSV4 stages completed and were accepted on frozen
`post_dsv4_live_cap_v1`: CSA context 88/88 tasks at 20,296/20,392 exact keys
(precisely 96 source-capacity omissions), HCA context 88/88 at 20,296/20,392
with the same 96, and both generation paths 88/88 at 1,600/1,600 each —
43,792 unique positive rows total, clean postflights. Validator iterations
`5738d4a2...` → `2cd2b85c...` → final `72dd57b4...` were validation-contract
corrections only (zero execution-path or plan-input delta); the final
validator requires one positive max/full/SWA pool record per worker log.
Final source-bundle SHA256 is `023bd96f...`; the 775-file read-only evidence
manifest SHA256 is
`3fbf1181702fa70b7192a075b4b631dbb4e22127a6020b84d37c0ea51754597d`.
H20 execution evidence only — not H200/Blackwell data; SM100/103 unmeasured,
SM120 fail-closed.

### MoE final-stage readiness (2026-07-03) (stub, consolidated 2026-07-09)

MoE smoke-gate preparation and its runner-envelope iteration, superseded by
the alignment failure/prune entry below. The pre-alignment SM90 stock plan was
114,105 cases (45,684 BF16, 38,799 FP8-block, 20,493 NVFP4, 6,051 W4A16-MXFP4,
3,078 INT4-WO). The first hardware smoke retained 48/64 rows because
TensorRT-LLM could not create `/root/.tensorrt_llm/tmp` on the read-only
container root while Docker/collector still exited 0; after adding the
task-local TensorRT cache mount and exact 8/0/0 (full: 114,105/0/0) checkpoint
gates, the complete 64-key smoke passed (accepted-smoke manifest
`1309fdb9...`, failure manifest `dca7abfe...` retained as diagnostic).

### MoE SM90 full alignment failure and approved prune (2026-07-03) (narrative compressed 2026-07-09)

Full attempt v1 processed all 114,105 tasks but is rejected: 113,376 done,
729 failed. Every failure is Nano BF16 `flashinfer_cutlass` with intermediate
size 1856: TP16 produces local inter 116 and TP32 produces 58, both violating
the exact CUTLASS `%8` contract; the failed grid is 27 token sizes × three
distributions × nine TP/EP pairs, with no OOM, worker crash, restart, or
environment failure. Frozen failed-attempt manifest SHA256 is
`eeadf647806568e056cf5eb0902a28336222d6a0a380b947510fd3b81931ce63`.
Classification: **current uncommitted integration omission / not introduced by
Blackwell** — pinned SGLang selects CUTLASS for SM90 Nano BF16 without a TP
fallback, FlashInfer requires eight-element alignment, and `c8c00f42` had
matching plan/runtime guards while the current integration omitted the plan
guard.

Approved prune decision: the user approved skipping the uncommon TP16/32
slices; the implemented contract is deliberately narrower — prune only BF16
cases whose resolved backend is `flashinfer_cutlass` and whose hidden or
TP-local inter dimension is not divisible by 8. This removes exactly the 729
failed IDs and preserves Nano TP1/2/4/8 plus same-backend Ultra TP16/32
positive controls; new SM90 plan target 113,376. SM100/103 are unaffected
(Nano uses `flashinfer_trtllm` there); SM120 resolves to CUTLASS and must
revalidate the same backend-aware predicate on hardware.

Outcome: from frozen `post_moe_cutlass_align_v1` (manifest `f22a01d0...`),
the 96-row v2 smoke passed 96/96 and the fresh no-resume full run completed
113,376/0/0 with strict validation (task SHA `dfa28d4d...`, consumer-key SHA
`b243dd0d...`, CSV/checkpoint `3de99f97...`/`102351b8...`). Reverse
measurement validation against rejected v1: identical 113,376 keys/sources,
latency v2/v1 p01 0.8938, median 1.0000, p99 1.1194 (all rows inside 0.5–2;
comparison manifest `c20cfb65...`), proving the plan guard changed only
enumeration. An H200/0.5.10 diagnostic comparison (43,497 common keys, 2,349
Triton→CUTLASS from the audited Nano selector change) does not authorize H200
relabeling. Note: this 113,376-row artifact was later semantically rejected by
the MoE FP4/INT4 identity reversal (all 20,493 NVFP4 rows used
`sglang_marlin_moe`); see the `final_v1` stub below.

### final_v1 collector-only candidate status binding (2026-07-04) (stub, consolidated 2026-07-09)

Execution root `/raid/aic_sglang_0514_sm90_final_20260704`, clean
execution-code HEAD `1b37712659554e834ac520a18eb1c0620a0e16df`, read-only
`source_final_v1/code_snapshot` manifest-file SHA256
`2c53ee9d73c1364c471024ca8ee94b6e5578ce7444c6c4a99f55a72f0c50f3bc`, on eight
H20-3e GPUs at SM90 (`h200_sxm` is only the existing plan selector; no H20
row may be published or relabeled as H200 or Blackwell data). MoE chronology:
`114,105 pre-alignment -> 113,376 alignment-pruned -> 92,883 final target`
(the 113,376-row artifact is semantically rejected: all 20,493 NVFP4 rows
used `sglang_marlin_moe`). Accepted fresh `final_v1` evidence: dense attention
50,901 context / 40,468 generation / 7,679 encoder rows; DSA modules 264
context tasks / 46,688 rows and 24 generation tasks / 3,568 rows (no
skip-indexer rows); GLM sparse 3,025 MQA / 6,050 PAGED top-k / 3,025
DSA-attention rows for BF16 `zai-org/GLM-5`; and a 64-row MoE smoke with
Marlin only on INT4-WO and no NVFP4 row. The in-progress 92,883-row MoE full
run was stopped by the PR #1302 rebase (next stub) and never accepted.

### PR #1302 rebase transition (2026-07-04) (stub, consolidated 2026-07-09)

Now historical: PR #1302 merged upstream (squash `7a4ca384`) and the branch
was rebased onto it on 2026-07-09. The original 2026-07-04 transition rebased
the collector-only branch from base `9ce84ebb` onto PR #1302 head
`4c6cfebdc5bfbfd28f15ebad4aa9d17199f94b3a` (first rebased SGLang head
`28c283cd`); #1302 retires the selector/`sm_exceptions` mechanism, adds
positive capability floors and hang-only denylisting, and binds checkpoints
to framework/SM identity, so no row from `source_final_v1` validates the
rebased product. The only retained worker delta converts an operation's
successful integer `EXIT_CODE_RESTART` return into PR #1302's explicit
restart path. The interrupted `final_v1` MoE namespace
`runs/moe_full_pre_pr1302_stale_20260704T211655` (7,116/92,883 tasks done,
7,137 partial rows) is chronology only and must not be resumed, filtered,
finalized, or published. Dense, DSA, and GLM full results remain accepted
evidence only for frozen snapshot `1b377126...`/manifest `2c53ee9d...`.

### Post-#1302 rule audit (2026-07-04; executed and landed — stub, consolidated 2026-07-09)

The rule audit proposed on 2026-07-04 against snapshot `28c283cd` (backup ref
`backup/sglang-0514-post-pr1302-pre-rule-audit-20260704`) was subsequently
implemented and committed on the branch (commits `bdd80363`/`6f85f6f8`/
`5bfa10be` etc. carry it). Durable outcomes: the `f9c1e29c` fixed GDN context
token/value thresholds and the pre-backend Blackwell FP8-live-attention SM
drop were removed (TRTLLM-MHA/FlashInfer paths raise fail-closed instead of
logging false FP8 rows); the `fa65531a`/`f9c1e29c` MoE alignment, DSV4 FP4
EP/TP/token/SM, and backend-limit generation filters were removed (stable
`allowed_modes`, universal MoE mathematics, and physical-key dedup remain; the
shared Qwen `max_tp_exclusive` declaration was intentionally unchanged); the
inherited `37826f10` SM120 DeepGEMM-module early return became a #1302
registry maturity marker with the getter still executable. Fail-loud/
provenance changes landed with it: `log_perf` no-row is a failure across GEMM/
attention/MLA/mHC/GDN/MoE paths; GLM/DSV4 sparse benchmarks no longer fall
back silently from CUDA Graph to eager; GEMM/MLA-BMM/mHC rows name their
invoked SGLang/Torch/TileLang path; DSA module rows derive a composite source
from the initialized backend (`use_mha`, `dsa_prefill_impl`,
`dsa_decode_impl`); framework-layer MoE validates the constructed quant
method/runner before timing (DSV4 `w4a16_mxfp4` on SM90 constructs
`Fp8MoEMethod` and now fails before timing — later resolved by the DSV4-Pro
w4a16 non-support decision below). One mechanism boundary was deliberately
not crossed at that checkpoint — moving DSA/DSV4 grouped-inner structure and
live-memory feasibility before queueing requires owner approval; the DSV4
part was later approved and implemented (next stub), while the DSA context
worker-side `2^25`/chunk/pool filters remain the separately tracked mechanism
debt.

### DSV4 context hidden-state feasibility checkpoint (2026-07-04) (stub, consolidated 2026-07-09)

Owner-approved narrow grouped-inner change, implemented and committed: a DSV4
CSA/HCA context inner shape is filtered before queueing when the mandatory
BF16 input hidden state alone — `batch_size * sequence_length * hidden_size *
2` bytes, budget 80% of the smallest visible CUDA device's reported total
memory, equality retained — exceeds the live device-capacity budget. A
canonical `((prefix, (sequence_lengths...)), ...)` manifest is attached to
every retained context outer task so the scheduler ID and resume checkpoint
change exactly when the executable inner set changes; generation is unchanged.
On H20 (150,110,011,392 bytes/device; budget 120,088,009,113) each context op
drops 4,504 of 56,936 expanded inner cells (7.91%) to 52,432 retained; a live
2026-07-05 getter probe reproduced 88 outer tasks, 52,432 retained cells, and
a batch-1024 prefix-zero ceiling of 12,288 for both CSA and HCA. Only the
executing platform supplies the budget; no H20 capacity result may be copied
to Blackwell. Old DSV4 context checkpoints have a different task identity and
must not be resumed. The release owner allowed the commit without a clean
smoke (the first smoke was contaminated by an unrelated vLLM container); an
uncontaminated smoke in a fresh namespace remains required before an accepted
full collection begins. Exact before/after task and expanded-inner hashes are
