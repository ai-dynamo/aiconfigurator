...
# The unrolled sample (& how to override it)
...
| `backend` | `selection["backend"]` (`vllm` / `sglang` / `trtllm`) |
...
`planner_scaling_policy` decodes via `scaling_fields` (`src/spica/planner.py`). (For a
...
(`src/spica/load_predictor_sweep.py`): always `load_predictor` (family) +
...
`search_space:` YAML. The first two are detailed in [search-space.md](search-space.md);
...
and `used_gpus` remain derived. See [search-space.md](search-space.md) for the full shape
...
unless pinned via mechanism #3. See [search-space.md](search-space.md#kinds-of-knob).
