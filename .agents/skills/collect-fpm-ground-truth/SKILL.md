...
Prefer one deployment per sweep. Use `--workload-plan sweep` and collect `context,decode,mixed` in the same Dynamo stack. Always preserve `*_phase.csv`, `*_detail.csv`, and `*_workload.csv`; mixed rows are needed for AIC aggregate validation.
...
- For context repeats, summarize with gymnastics scoring: drop min and max, then average.
- For decode, compare pure decode-only FPM rows by `decode_requests` and `mean_decode_kv_tokens`.
