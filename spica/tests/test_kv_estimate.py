...
"""KV-cache feasibility wrapper around AIC's ``estimate_kv_cache``.

The filtering logic is unit-tested by monkeypatching ``estimate_kv_cache`` (no
perf DB needed). Two skip-guarded integration tests exercise the real native
estimate when a perf DB is present."""
...
def test_estimate_kv_tokens_returns_capacity(monkeypatch):
    monkeypatch.setattr("spica.kv_estimate.estimate_kv_cache", lambda *a, **k: {"total_kv_size_tokens": 123456})
...
def test_estimate_kv_tokens_oom_returns_none(monkeypatch):
    ...
    monkeypatch.setattr("spica.kv_estimate.estimate_kv_cache", boom)
...
def test_feasible_shape_tokens_filters_short_and_oom_and_dedups(monkeypatch):
    ...
    monkeypatch.setattr("spica.kv_estimate.estimate_kv_cache", fake)
...
def test_feasible_shape_tokens_resolves_version_when_missing(monkeypatch):
    monkeypatch.setattr("spica.kv_estimate.get_latest_database_version", lambda *a, **k: None)
...
def test_real_estimate_dense_qwen_feasible():
    ...
    try:
        tokens = estimate_kv_tokens(
            sh, model_name="Qwen/Qwen3-32B", hardware_sku="h200_sxm", backend="trtllm", backend_version=ver
        )
...
def test_real_estimate_deepseek_two_gpus_oom():
    ...
    try:
        tokens = estimate_kv_tokens(
            sh, model_name="deepseek-ai/DeepSeek-V3", hardware_sku="gb200", backend="trtllm", backend_version=ver
        )
    except ValueError as exc:
        _skip_if_native_unavailable(exc)
