...
try:
    ...
    HAS_SGL_FP8_KERNEL = True
except ImportError:
    fp8_scaled_mm = None
    sgl_per_token_quant_fp8 = None
    HAS_SGL_FP8_KERNEL = False
...
try:
    ...
    DEEPGEMM_SCALE_UE8M0 = None
    gemm_nt_f8f8bf16 = None
...
def _device_name_for_log(device: str) -> str:
...
def get_gemm_test_cases():
    ...
    if is_hip() or _runtime_hip_available():
        # ROCm path: keep collector stable with torch.float16 GEMM.
        gemm_list = ["float16"]
    else:
...
def run_gemm(gemm_type, batch_size, N, K, perf_filename, device):  # noqa: N803
    ...
    on_hip = is_hip() or _runtime_hip_available()
    ...
    def create_gemm():
        ...
        if gemm_type == "nvfp4":
            ...
            if on_hip or not HAS_SGL_FP8_KERNEL:
                return None
            fp8_info = torch.finfo(torch.float8_e4m3fn)
            a_bf16 = torch.randn(M, K, dtype=dtype, device=device)
            b_fp32 = (torch.rand(N, K, device=device) - 0.5) * 2 * fp8_info.max
    ...
    has_nvtx = hasattr(torch.cuda, "nvtx") and hasattr(torch.cuda.nvtx, "range_push")
    if has_nvtx:
    ...
    log_perf(
        ...
        device_name=_device_name_for_log(device),
        op_name="gemm",
