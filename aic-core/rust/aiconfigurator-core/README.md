...
- MoE attention-DP token scaling (SGLang all-gathers DP-sharded tokens before
  ...
  to Python.
- Large-EP MoE: `MoeAllToAll` reads measured cross-node communication from
  `moe_a2a_perf`; the schema-v7 wire name `EpMoe` executes
  `ModeledEpMoeOp`, which estimates balanced local expert compute from stock
  `moe_perf`.
