            ...
            "identity": json.dumps(
                {
                    ...
                    # (model_path, ModelConfig) — e.g. recipe-built models
                    # sharing a checkpoint path with the registry class —
                    # declare a discriminator via ``engine_identity_extra``;
                    # without it two structurally different models share one
                    # cached handle and silently return each other's latencies
                    # (same failure mode the forward_model key guards against).
