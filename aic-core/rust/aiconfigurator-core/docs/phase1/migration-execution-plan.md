...
`aic-core/rust/aiconfigurator-core/parity_tests/`. After Phase 3 (C8-C10) all 12
...
`aic-core/rust/aiconfigurator-core/parity_tests/benchmark_engine_step.py`.
...
1. Commit the working-tree clarification in
   `aic-core/rust/aiconfigurator-core/docs/phase1/migration-map.md` that defines what "target
   Rust path" means during the transition.
2. Update `aic-core/rust/aiconfigurator-core/docs/phase1/migration-map.md`:
...
3. Capture a baseline benchmark snapshot to
   `aic-core/rust/aiconfigurator-core/parity_tests/benchmarks.md` so Phase 3 progress
...
AICONFIGURATOR_RUST_CORE_AUTOBUILD=1 uv run pytest \
  aic-core/rust/aiconfigurator-core/parity_tests/test_engine_step_parity.py -p no:xdist
```
