    ...
    *,
    ...
    if allow_transformers_fallback and not config and not tokenizer_config:
        ...
        if not config and not tokenizer_config:
            raise ValueError(f"could not determine random prompt token vocabulary for {model!r}") from fallback_error
    else:
    ...
    raw_vocab_size = config.get("vocab_size") or text_config.get("vocab_size") or tokenizer_config.get("vocab_size")
